...
function Bot(token, config, webhook) {
	this.token = token;
	this.config = config;
	this.highestId = 0;
	...
	request.post({
		...
		if(!error && response.statusCode == 200) {
			...
			if(apiResponseObj.ok) {
				...
				setTimeout(function () {
					self._api(method, args, callback, apiResponseObj.description);
				}, 1000);
			}
		} else {
			var currentError;
			...
			setTimeout(function () {
				self._api(method, args, callback, currentError);
			}, 1000);
		}
	});
};
...
Bot.prototype.resetWebhook = function() {
