	...
	request.post({url: 'https://api.telegram.org/bot' + this.token + '/' + method, form: args}, function(error, response, body){
		if (!error && response.statusCode == 200) {
			...
			if(apiResponseObj.ok){
				if(previousError) console.log("%s: Connection working again", self.me.first_name);
				if(typeof callback == 'function'){
					callback(apiResponseObj.result);
				}
			}else{
				if(apiResponseObj.description != previousError) console.error('%s: API Error: %s', self.me.first_name, apiResponseObj.description);
				setTimeout(function(){self._api(method, args, callback, apiResponseObj.description);}, 1000);
			}
		}else{
			var currentError;
			if(error){
				if(error.toString() != previousError) console.error('%s: Connection %s', self.me.first_name, error);
				currentError = error.toString();
			}else if(response && response.statusCode != 200){
				if(response.statusCode != previousError) console.error("%s: Server Error: StatusCode %s", self.me.first_name, response.statusCode);
